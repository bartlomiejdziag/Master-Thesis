...
static err_t app_callback_received(void *arg, struct tcp_pcb *tpcb, struct pbuf *p, err_t err)
{
  ...
  else if (es->state == ES_ACCEPTED) //first data receiving
  {
	...
	// Parse packet
		if (*(uint8_t*) es->p->payload == '1') {
			HAL_GPIO_WritePin(LD2_GPIO_Port, LD2_Pin, GPIO_PIN_SET); //turn on blue LED when "1" is received.
		} else if (*(uint8_t*) es->p->payload == '0') {
			HAL_GPIO_WritePin(LD2_GPIO_Port, LD2_Pin, GPIO_PIN_RESET); //turn off blue LED when '0' is received.
		}
    tcp_sent(tpcb, app_callback_sent); //register send callback
