...
void StartLed2Task(void *argument)
{
  /* USER CODE BEGIN StartLed2Task */
	...
	tick = xTaskGetTickCount();        // retrieve the number of system ticks
	/* Infinite loop */
	for (;;) {
		HAL_GPIO_TogglePin(LD2_GPIO_Port, LD2_Pin);
		printf("LED2\n\r");
		vTaskDelayUntil(&tick, (300U * configTICK_RATE_HZ) / 1000);
	}
...
void StartLed3Task(void *argument)
{
  ...
  for(;;)
  {
		...
		printf("LED3\n\r");
		tick += ((500U * osKernelGetTickFreq()) / 1000);                      // delay 300 ticks periodically
		osDelayUntil(tick);
  }
